...
# %% softlink all data to bids_dir

# raw data
subprocess.call('ln -s {0} {1}/{2}'.format(raw_data, bids_dir, 'rawdata'), shell=True)
...
for sub in sublist:
    ...
    subprocess.call('ln -s {0}/sub-{1}/ses-movie/anat {2}/anat'.format(preproc_mri, sub, bids_preproc_ses_dir), shell=True)
    subprocess.call('ln -s {0}/sub-{1}/ses-movie/meg {2}/meg'.format(preproc_meg, sub, bids_preproc_ses_dir), shell=True)
    
    ses_ass_f = glob.glob(os.path.join(preproc_meg, 'sub-{0}'.format(sub), 'ses-movie', ses_ass_files[0]))[0]
    subprocess.call('ln -s {0} {1}'.format(ses_ass_f, bids_preproc_ses_dir), shell=True)

# copy study associated files
for f in study_ass_files:
    subprocess.call('cp {0}/{1} {2}'.format(raw_data, f, bids_dir), shell=True)
